"""
Google Ads Competitor Finder v7
- Dùng ValueSerp thay SerpAPI (35,000 lần/tháng chỉ $10)
...
app = FastAPI(title="Google Ads Competitor Finder v7")
...
async def valueserp_get(params: dict) -> dict:
    """Gọi ValueSerp API"""
    async with httpx.AsyncClient(timeout=30) as client:
        resp = await client.get(
            "https://api.valueserp.com/search",
            params={**params, "api_key": VALUESERP_KEY}
        )
        return resp.json()
        ...
        try:
            ...
            for ad in data.get("ads", []):
                ...
                if not domain or domain in seen:
                    continue
                seen.add(domain)
                sitelinks = [sl.get("title","") for sl in (ad.get("sitelinks") or []) if isinstance(sl,dict)]
                results.append(make_result(
                    domain, ad.get("title",""), ad.get("description",""),
                    ad.get("displayed_link",""), ad.get("link",""),
                    sitelinks, ad.get("extensions",[]),
                    "Google Ads", True, q, country.upper()
                ))
            ...
            for org in data.get("organic_results",[])[:4]:
                ...
                seen.add(domain)
                results.append(make_result(
                    domain, org.get("title",""), org.get("snippet",""),
...
async def analyze_one_country(url: str, country: str, seen: set):
    ...
    try:
        data = await valueserp_get({"q": f"site:{domain_target}", "num": 3})
    ...
    queries = [
        f"{domain_target} competitor",
        f"alternative to {domain_target}",
        f"sites like {domain_target}",
        title_kw,
        f"best {title_kw}",
        f"{domain_target} vs",
        f"{title_kw} review",
    ...
    for q in queries:
        ...
        try:
            ...
            for ad in data.get("ads", []):
                ...
                sitelinks = [sl.get("title","") for sl in (ad.get("sitelinks") or []) if isinstance(sl,dict)]
                results.append(make_result(
                    d, ad.get("title",""), ad.get("description",""),
                    ad.get("displayed_link",""), ad.get("link",""),
                    sitelinks, ad.get("extensions",[]),
                    "Google Ads", True, q, country.upper()
                ))
            ...
            for org in data.get("organic_results",[])[:3]:
                ...
                seen.add(d)
                results.append(make_result(
                    d, org.get("title",""), org.get("snippet",""),
                    org.get("displayed_link",""), org.get("link",""),
                    [], [], "Organic", False, q, country.upper()
                ))
...
@app.get("/")
async def root():
    return {"status": "online", "version": "7.0", "engine": "ValueSerp"}

@app.get("/health")
async def health():
    return {
        "status": "ok",
        "serpapi_configured": bool(VALUESERP_KEY),
        "engine": "ValueSerp"
    }
...
async def find_similar(
    ...
    if not VALUESERP_KEY:
        return {"error": "Chưa có VALUESERP_KEY", "total": 0, "results": []}
